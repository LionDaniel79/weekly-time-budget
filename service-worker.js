...
const SHELL_URLS = [
  ...
  './src/view-change-events.js', './src/auth-login-guard.js', './src/category-ui-patch.js', './src/category-selection-memory.js',
  ...
  './src/statistics-state.js', './src/statistics-data-source.js', './src/statistics-view.js', './src/statistics-feature.js',
  './src/statistics-bootstrap.js', './src/persistent-timer.js', './src/persistent-timer-ui.js', './src/offline-entry-domain.js',
  './src/offline-store.js', './src/offline-entry-repository.js', './src/offline-sync.js', './src/offline-runtime.js',
...
function isSensitiveApi(url) {
  return ['firestore.googleapis.com', 'identitytoolkit.googleapis.com', 'securetoken.googleapis.com', 'accounts.google.com']
    .some((host) => url.hostname === host || url.hostname.endsWith(`.${host}`));
}
const isFirebaseModule = (url) => url.href.startsWith(FIREBASE_VERSION_ROOT);
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    const runtime = await caches.open(RUNTIME_CACHE);
    await Promise.allSettled([cacheModuleGraph({
