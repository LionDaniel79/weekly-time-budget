...
const optionHtml = () => state.categories.map((category) => `<option value="${category.id}">${escapeHtml(category.name)}</option>`).join('');
...
async function saveCategory({ id, name, budgetMinutes }) {
  const payload = { name: name.trim(), budgetMinutes: Number(budgetMinutes) || 0, order: state.categories.findIndex((c) => c.id === id) + 1 || state.categories.length + 1 };
...
function renderRecord() {
  ...
  document.querySelectorAll('[data-record-tab]').forEach((button) => button.onclick = () => { state.activeRecordTab = button.dataset.recordTab; renderRecord(); });
...
function timerForm() {
  ...
  return `<div class="form-grid"><label>대분류<select id="timer-category" ${state.timer ? 'disabled' : ''}><option value="">선택하세요</option>${optionHtml()}</select></label><label>메모(선택)<textarea id="timer-note" rows="2" ${state.timer ? 'disabled' : ''}></textarea></label></div><div id="timer-display" class="timer">${formatClock(elapsed)}</div><div class="actions"><button id="timer-action" class="primary-button">${state.timer ? '종료하고 저장' : '타이머 시작'}</button>${state.timer ? '<button id="timer-cancel" class="secondary-button">취소</button>' : ''}</div>`;
}
  ...
  action.onclick = async () => {
    if (!state.timer) {
      ...
      state.timerInterval = setInterval(() => {
        const display = $('#timer-display'); if (display) display.textContent = formatClock(Math.floor((Date.now() - state.timer.startedAt) / 1000));
      }, 1000);
    } else {
      const durationMinutes = Math.max(1, Math.round((Date.now() - state.timer.startedAt) / 60000));
      const timer = state.timer; clearInterval(state.timerInterval); state.timer = null;
      await saveEntry({ categoryId: timer.categoryId, note: timer.note, date: toDateKey(new Date()), durationMinutes, startTime: new Date(timer.startedAt).toTimeString().slice(0,5), endTime: new Date().toTimeString().slice(0,5), source: 'timer' });
    }
  };
...
function manualForm() {
  const now = new Date(); const end = now.toTimeString().slice(0,5); const startDate = new Date(now.getTime() - 60 * 60 * 1000); const start = startDate.toTimeString().slice(0,5);
  return `<form id="manual-form" class="form-grid"><label>대분류<select id="manual-category" required><option value="">선택하세요</option>${optionHtml()}</select></label><div class="grid grid-2"><label>날짜<input id="manual-date" type="date" value="${toDateKey(now)}" required></label><label>시작<input id="manual-start" type="time" value="${start}" required></label><label>종료<input id="manual-end" type="time" value="${end}" required></label></div><label>메모(선택)<textarea id="manual-note" rows="2"></textarea></label><button class="primary-button" type="submit">기록 저장</button></form>`;
}

function bindManual() {
  $('#manual-form').onsubmit = async (event) => {
    event.preventDefault(); const durationMinutes = minutesBetween($('#manual-start').value, $('#manual-end').value);
    if (durationMinutes <= 0 || durationMinutes > 1440) return alert('시간 범위를 확인하세요.');
    await saveEntry({ categoryId: $('#manual-category').value, note: $('#manual-note').value.trim(), date: $('#manual-date').value, durationMinutes, startTime: $('#manual-start').value, endTime: $('#manual-end').value, source: 'manual' });
  };
}

function renderBudget() {
  $('#budget-view').innerHTML = `<div class="card"><div class="section-title"><div><h2>주간 시간 예산</h2><p class="muted">예산은 월요일부터 토요일까지만 적용됩니다.</p></div></div>${state.categories.length ? `<div class="category-list">${state.categories.map((category) => `<form class="category-item budget-form" data-id="${category.id}"><strong>${escapeHtml(category.name)}</strong><input type="number" name="hours" min="0" step="0.5" value="${(category.budgetMinutes || 0) / 60}" aria-label="${escapeHtml(category.name)} 예산 시간"><button class="secondary-button">저장</button></form>`).join('')}</div>` : $('#empty-template').innerHTML}</div>`;
  document.querySelectorAll('.budget-form').forEach((form) => form.onsubmit = async (event) => { event.preventDefault(); const category = state.categories.find((item) => item.id === form.dataset.id); await saveCategory({ ...category, budgetMinutes: Number(new FormData(form).get('hours')) * 60 }); });
}

function renderHistory() {
  $('#history-view').innerHTML = `<div class="card"><div class="section-title"><h2>최근 기록</h2><span class="badge">${state.entries.length}건</span></div>${state.entries.length ? state.entries.map((entry) => { const category = state.categories.find((item) => item.id === entry.categoryId); return `<div class="entry"><strong>${entry.date}</strong><div><strong>${escapeHtml(category?.name || '삭제된 대분류')}</strong><div>${entry.startTime || ''}–${entry.endTime || ''} · ${formatMinutes(entry.durationMinutes)}</div>${entry.note ? `<p class="muted">${escapeHtml(entry.note)}</p>` : ''}</div><div class="entry-actions"><button class="text-button delete-entry" data-id="${entry.id}">삭제</button></div></div>`; }).join('') : '<div class="empty-state"><h3>아직 기록이 없습니다.</h3><p>타이머 또는 수동 입력으로 첫 시간을 기록하세요.</p></div>'}</div>`;
  document.querySelectorAll('.delete-entry').forEach((button) => button.onclick = () => deleteEntry(button.dataset.id));
}

function renderCategories() {
  $('#categories-view').innerHTML = `<div class="grid grid-2"><div class="card"><h2>대분류 추가</h2><form id="category-add" class="form-grid"><label>이름<input name="name" placeholder="예: 논문" required maxlength="30"></label><label>기본 주간 예산(시간)<input name="hours" type="number" min="0" step="0.5" value="0"></label><button class="primary-button">추가</button></form></div><div class="card"><h2>등록된 대분류</h2>${state.categories.length ? `<div class="category-list">${state.categories.map((category) => `<form class="category-item category-edit" data-id="${category.id}"><input name="name" value="${escapeHtml(category.name)}" required><input name="hours" type="number" min="0" step="0.5" value="${(category.budgetMinutes || 0) / 60}"><div class="actions"><button class="secondary-button">저장</button><button type="button" class="danger-button category-delete">삭제</button></div></form>`).join('')}</div>` : $('#empty-template').innerHTML}</div></div>`;
  $('#category-add').onsubmit = async (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); await saveCategory({ name: data.get('name'), budgetMinutes: Number(data.get('hours')) * 60 }); };
  document.querySelectorAll('.category-edit').forEach((form) => { form.onsubmit = async (event) => { event.preventDefault(); const data = new FormData(form); await saveCategory({ id: form.dataset.id, name: data.get('name'), budgetMinutes: Number(data.get('hours')) * 60 }); }; form.querySelector('.category-delete').onclick = () => deleteCategory(form.dataset.id); });
}

function formatClock(seconds) { const h = Math.floor(seconds / 3600); const m = Math.floor(seconds % 3600 / 60); const s = seconds % 60; return [h,m,s].map((v) => String(v).padStart(2,'0')).join(':'); }
...
$('#google-login').onclick = async () => { const provider = new firebase.GoogleAuthProvider(); await firebase.signInWithPopup(auth, provider); };
$('#logout').onclick = () => firebase.signOut(auth);

initFirebase().catch((error) => { console.error(error); $('#config-warning').textContent = `초기화 오류: ${error.message}`; $('#config-warning').classList.remove('hidden'); });
