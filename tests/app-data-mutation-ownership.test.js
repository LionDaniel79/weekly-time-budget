...
test('대분류와 원격 기록 쓰기는 app data source가 소유한다', async () => {
  const [app, entryService, dataSource] = await Promise.all([
    read('src/app.js'), read('src/app-entry-service.js'), read('src/app-data-source.js'),
  ]);
  ...
  assert.match(entryService, /dataSource\.deleteEntry\(/);
