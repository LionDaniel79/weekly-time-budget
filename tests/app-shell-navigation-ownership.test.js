...
test('앱 셸은 메뉴 클릭을 단일 delegated handler에서 처리한다', async () => {
  const source = await read('src/app-shell.js');
  assert.match(source, /document\.addEventListener\('click',[\s\S]*\.nav-button\[data-view\]/);
...
test('모바일 메뉴는 접근성 상태와 사이드바 상태를 함께 갱신한다', async () => {
  ...
  assert.match(source, /closeSidebar: false/);
});
