...
test('서비스 워커는 앱 셸과 Firebase 런타임 캐시를 분리한다', async () => {
  ...
  for (const token of ['weekly-time-budget-shell-v17', 'weekly-time-budget-firebase-v2', 'firebaseCacheFirst', 'shellCacheFirst', 'firestore.googleapis.com', "request.mode === 'navigate'", './src/statistics-feature.js', './src/statistics-bootstrap.js']) assert.ok(serviceWorker.includes(token), token);
