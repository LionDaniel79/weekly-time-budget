...
test('Pages workflow는 최소 권한과 최신 배포 우선 동시 실행 제어를 선언한다', async () => {
  const source = await workflow();
...
test('Pages workflow는 release 검증 후 Firebase 변수로 _site를 준비한다', async () => {
  const source = await workflow();
  assert.match(source, /Validate release modules/);
  assert.match(source, /Verify release source contracts/);
  ...
  for (const name of [
    ...
    assert.match(source, new RegExp(`${name}: \\\${{ secrets\\.${name} \\|\\| vars\\.${name} }}`));
  }
});
